...
def dashboard():
    ...
    return render_template(
        ...
        channel_total=(
            sum(
                channel_split.values()
            )
            or 1
        ),
        plan_split=plan_split,
        plans=current_app.config[
            "PLANS"
        ],
...
def nfc_orders():
    ...
    if search:
        query = query.filter(
            db.or_(
                NFCOrder.order_number.ilike(
                    f"%{search}%"
                ),
                NFCOrder.full_name.ilike(
                    f"%{search}%"
                ),
                NFCOrder.email.ilike(
                    f"%{search}%"
                ),
            )
        )
...
@admin_required
def nfc_order_detail(order_id):
    """View one NFC order."""

    nfc_order = db.session.get(
        NFCOrder,
        order_id,
    )
...
@bp.route(
    "/nfc/<int:order_id>/status",
    methods=["POST"],
)
@admin_required
def update_nfc_status(order_id):
    """Update NFC production/delivery status."""
    ...
    if new_status not in NFCOrder.STATUSES:
        flash(
            "Invalid NFC order status.",
            "error",
        )
    ...
    nfc_order.status = new_status
    nfc_order.updated_at = utcnow()

    db.session.commit()

    flash(
        f"{nfc_order.order_number} is now "
...
def clients():
    ...
    if state == "active":
        ...
        users = [
            u
            for u in users
            if not u.is_subscription_active
        ]
...
def toggle_suspend(user_id):
    ...
    flash(
        f"{user.email} is now "
        f"{'suspended' if user.is_suspended else 'active'}.",
        "info",
    )
...
def _gallery_extension(filename):
    ...
    if (
        extension
        not in ALLOWED_GALLERY_EXTENSIONS
    ):
        return None
...
def gallery():
    if request.method == "POST":
        ...
        flash(
            f"{original_name} was added to the homepage gallery.",
            "success",
        )
