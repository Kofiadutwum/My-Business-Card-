...
class BaseConfig:
    ...
    SQLALCHEMY_DATABASE_URI = os.environ.get(
        "DATABASE_URL",
        "sqlite:///" + os.path.join(BASE_DIR, "instance", "cardhub.db"),
    )
    ...
    USD_GHS_RATE = float(
        os.getenv("USD_GHS_RATE", "0.0869909")
    )

    GRACE_PERIOD_DAYS = int(
        os.environ.get("GRACE_PERIOD_DAYS", 7)
    )

    RENEWAL_NOTICE_DAYS = int(
        os.environ.get("RENEWAL_NOTICE_DAYS", 30)
    )
    ...
    PAYMENT_SANDBOX_OUTCOME = os.environ.get(
    ...
    # --- NFC admin notifications ----------------------------------------

    NFC_ADMIN_EMAIL = os.environ.get(
        "NFC_ADMIN_EMAIL",
    ...
    SMS_CLIENT_ID = os.environ.get(
        "SMS_CLIENT_ID",
        "",
    )

    SMS_CLIENT_SECRET = os.environ.get(
        "SMS_CLIENT_SECRET",
        "",
    )
    ...
    LEADS_PER_VISITOR_PER_HOUR = int(
        os.environ.get(
            "LEADS_PER_VISITOR_PER_HOUR",
            3,
        )
    )
    ...
    SITE_SHORT_NAME = os.environ.get(
        "SITE_SHORT_NAME",
        "AD Smart",
    )
    ...
    SUPPORT_EMAIL = os.environ.get(
        "SUPPORT_EMAIL",
        "adgraphics881@gmail.com",
    )

    SUPPORT_PHONE = os.environ.get(
        "SUPPORT_PHONE",
        "0545875881",
    )
